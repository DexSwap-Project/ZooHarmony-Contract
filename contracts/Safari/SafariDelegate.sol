...
contract SafariDelegate is SafariStorage, Initializable, AccessControl {
    ...
    event QuitDexSwapDex(address to, uint256 amount);
    event EmergencyQuitDexSwapDex(address to, uint256 amount);

    function initialize(address admin, IWONE _wone) public payable initializer {
    ...
    // View function to see pending DexSwapDex on frontend.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            if (pool.rewardToken == address(wone)) { // convert wone to WONE/ONE 
                wone.withdraw(pending);
    ...
    receive() external payable {
        require(msg.sender == address(wone), "Only support value from WONE"); // only accept ONE via fallback from the WONE contract
    }
}
