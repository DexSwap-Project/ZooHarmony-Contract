...
contract ZooHelperDelegate is Initializable, AccessControl, ZooHelperStorage {
    ...
    function config(address _zooHarmony, address _zooFarming, address _zooPair, address _nftFactory, address _safari) public {
    ...
    function getTotalScore(address user) public view returns (uint256) {
        uint total = 0;
        // wallet balance
        total = total.add(IERC20(zooHarmony).balanceOf(user));

        // pending zoo
        total = total.add(getPendingZoo(user));

        // farming zoo in zoo/xdexs pool
    ...
    function getFarmingZoo(address user) public view returns (uint256) {
        ...
        if (amount > 0) {
            ...
            uint reserve0;
            (reserve0, , ) = IDexSwapPair(zooPair).getReserves();
