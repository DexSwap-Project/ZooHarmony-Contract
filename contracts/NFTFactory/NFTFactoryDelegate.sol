...
contract NFTFactoryDelegate is Initializable, AccessControl, ERC721Holder, NFTFactoryStorageV2 {
    ...
    function initialize(address admin, address _zooHarmony, address _zooNFT) public payable initializer {
    ...
    function configTokenAddress(address _zooHarmony, address _zooNFT) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        zooHarmony = _zooHarmony;
        zooNFT = _zooNFT;
    }
    ...
    function buyGoldenChest() public {
        ...
        IERC20(zooHarmony).transferFrom(msg.sender, address(this), currentPrice);
        IZooHarmony(zooHarmony).burn(currentPrice);
        uint tokenId;
        uint level;
        uint category;
        uint item;
    ...
    function stakeZoo(uint _type) public {
        ...
        IERC20(zooHarmony).transferFrom(msg.sender, address(this), stakePrice);
    ...
    function stakeClaim(uint _type) public {
        ...
        IERC20(zooHarmony).safeTransfer(msg.sender, amount);
    ...
    function getRandomSeed() internal returns (uint) {
        ...
        IPrivateOracle(privateOracle).inputSeed(1);
